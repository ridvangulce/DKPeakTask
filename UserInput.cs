...
public class UserInput : MonoBehaviour
{
    ...
    private UpdateSprite _updateSprite;
    ...
    void Start()
    {
        ...
        _updateSprite = FindObjectOfType<UpdateSprite>();
    ...
    void Card(GameObject selected)
    {
        ...
        if (!selected.GetComponent<Selectable>().faceUp) // if the card clicked on is facedown
        {
            if (!Blocked(selected)) // if the card clicked on is not blocked
            {
                ...
                slot1 = this.gameObject;
                CameraShake.Instance.isShake = true;
                StartCoroutine(RotateCard());
            }
        }
        else if
            (selected.GetComponent<Selectable>()
             .inDeckPile) // if the card clicked on is in the deck pile with the trips
        {
    ...
    IEnumerator RotateCard()
    {
        ...
        {
            cardPrefab.transform.rotation = Quaternion.Euler(0f, i, 0f);
            cardPrefab.transform.localScale = new Vector3(2f, 2f, 2f);
            Debug.Log("IM FLIPPINGG");
    ...
    void Stack(GameObject selected)
    {
        ...
        slot1.transform.position = new Vector3(selected.transform.position.x, selected.transform.position.y - yOffset,
